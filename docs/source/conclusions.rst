...
- **Validity of the approach**
  Our results confirm that PLMs provide real value in recovering annotations with precision. The use of taxonomy and redundancy filters, together with additional metrics such as global and local identity and reliability index, strengthens the robustness of the method. In the current implementation, taxonomy filtering is exact-ID based, so benchmark-style exclusions should rely on explicit curated taxonomy lists when descendant expansion is intentionally disabled.
