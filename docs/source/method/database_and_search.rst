...
2. Build lookup tables in memory: per model, load `ids` and `embeddings`
   (optionally applying taxonomy include/exclude filters using the exact taxonomy IDs supplied
   in the config).
...
6. Write outputs: store raw per-accession CSV files grouped by model and layer.
   Post-processing computes reliability, scores, and final CSV/TSV artifacts. Results are
...
- MMseqs2 must be installed separately and available in ``PATH`` when redundancy filtering is enabled.
