...
- Optional **taxonomy include/exclude** filters and **query-aware redundancy masking** via MMseqs2.
- Taxonomy filtering currently works on the **exact taxonomy IDs** provided in the config.
- Descendant-based taxonomy expansion is currently disabled; explicit taxonomy IDs must be provided.
- MMseqs2 masking is heuristic and does **not** guarantee removal of all clearly similar donors; for
  stricter benchmark-style exclusion, use more neighbors plus explicit identity filtering.
...
- **Embedding-only**: stop after generating ``embeddings.h5`` and skip lookup/post-processing.
...
- **Lookup**: distance metric (cosine/euclidean), per-model thresholds, taxonomy filters, redundancy filter (MMseqs2), workers, CPU/GPU selection.
