...
def _normalize_distance_threshold_value(value):
    """Normalize per-model distance threshold values.

    Supported disabled forms:
    - ``None``
    - YAML booleans like ``false``
    - strings such as ``"false"``, ``"off"``, ``"disable"``, ``"disabled"``, ``"none"``, ``"null"``
    - legacy numeric ``0``

    Returns
    -------
    float | None
        ``None`` when disabled, otherwise a positive float threshold.
    """
    if value is None or value is False:
        return None
    if isinstance(value, str):
        lowered = value.strip().lower()
        if lowered in ("", "false", "off", "disable", "disabled", "none", "null"):
            return None
        value = lowered
...
def run_pipeline(conf):
    ...
    This function coordinates the entire functional annotation workflow:
      ...
      4. Optionally stops after the embedding step when ``only_embedding`` is enabled.
      5. Performs database lookup using the generated or provided embeddings.
    ...
    conf : dict
        ...
        - ``only_embedding`` (bool): If True, stop after the embedding step.
    ...
    try:
        ...
        if conf.get("only_lookup") and conf.get("only_embedding"):
            raise ValueError("'only_lookup' and 'only_embedding' cannot both be true.")

        if conf.get("only_lookup"):
            ...
            if conf.get("only_embedding"):
                logger.info(
                    "Embedding-only mode enabled. Skipping lookup stage. Embeddings saved at: %s",
...
def load_and_merge_config(args, unknown_args):
    ...
    # 3) Canonical mappings (mirror legacy/top-level and CLI flags into nested structure expected downstream)
    # 3.0 Legacy YAML compatibility: accept top-level `redundancy:` / `taxonomy:` blocks
    #     and forward them into `lookup.*` when nested keys are absent.
    legacy_redundancy = conf.get("redundancy")
    if isinstance(legacy_redundancy, dict):
        lk = conf.setdefault("lookup", {})
        r = lk.setdefault("redundancy", {})
        if "identity" in legacy_redundancy and "identity" not in r:
            r["identity"] = legacy_redundancy["identity"]
        if "coverage" in legacy_redundancy and "coverage" not in r:
            r["coverage"] = legacy_redundancy["coverage"]
        if "threads" in legacy_redundancy and "threads" not in r:
            r["threads"] = legacy_redundancy["threads"]

        # Keep flat duplicates for downstream components that still read flat keys.
        if conf.get("redundancy_filter") is None and "identity" in legacy_redundancy:
            conf["redundancy_filter"] = legacy_redundancy["identity"]
        if conf.get("alignment_coverage") is None and "coverage" in legacy_redundancy:
            conf["alignment_coverage"] = legacy_redundancy["coverage"]
        if conf.get("threads") is None and "threads" in legacy_redundancy:
            conf["threads"] = legacy_redundancy["threads"]

    legacy_taxonomy = conf.get("taxonomy")
    if isinstance(legacy_taxonomy, dict):
        lk = conf.setdefault("lookup", {})
        t = lk.setdefault("taxonomy", {})
        if "exclude" in legacy_taxonomy and "exclude" not in t:
            t["exclude"] = legacy_taxonomy["exclude"]
        if "include_only" in legacy_taxonomy and "include_only" not in t:
            t["include_only"] = legacy_taxonomy["include_only"]
        if "get_descendants" in legacy_taxonomy and "get_descendants" not in t:
            t["get_descendants"] = legacy_taxonomy["get_descendants"]

        # Keep flat duplicates for downstream components that still read flat keys.
        if conf.get("taxonomy_ids_to_exclude") in (None, []):
            conf["taxonomy_ids_to_exclude"] = legacy_taxonomy.get("exclude", [])
        if conf.get("taxonomy_ids_included_exclusively") in (None, []):
            conf["taxonomy_ids_included_exclusively"] = legacy_taxonomy.get("include_only", [])
    ...
    # 5) Normalize per-model distance thresholds so disabled states are explicit.
    for _, settings in (conf.get("embedding", {}).get("models", {}) or {}).items():
        if isinstance(settings, dict) and "distance_threshold" in settings:
            settings["distance_threshold"] = _normalize_distance_threshold_value(
                settings.get("distance_threshold")
            )

    # 6) Legacy compatibility: populate embedding.types with names of enabled models (YAML-only)
    ...
    # 7) Early validations for redundancy thresholds (if supplied)
    if ri is not None:
        iri = float(ri)
        if not (0.0 <= iri <= 1.0):
...
def main():
    ...
    - ``run``        : Execute the FANTASIA pipeline (embedding-only, lookup-only,
      or full embedding + lookup).
