...
Given an HDF5 file of query embeddings, the component:
  ...
  3) Selects nearest neighbors under configurable thresholds and optional query-aware
     redundancy masking.
...
- Optional query-aware redundancy masking using MMseqs2 clusters.
...
MMseqs2 is required only if query-aware redundancy masking is enabled.
...
class EmbeddingLookUp(GPUTaskInitializer):
    ...
    - MMseqs2-based redundancy masking is query-aware and heuristic: it masks donor sequences
      assigned to the same MMseqs2 cluster as the query, but does not guarantee exclusion of all
      clearly similar or high-identity donors.
    ...
    def __init__(self, conf, current_date):
        ...
        Args:
            conf (dict): Run configuration. Relevant keys include:
                ...
                - redundancy_filter (float): Identity threshold for MMseqs2 query-aware masking;
        ...
        self._q_idx_to_accessions: list[set[str]] = []
        ...
        self.query_index_mapping_path = os.path.join(
            self.experiment_path,
            self.conf.get("query_index_mapping", "query_index_mapping.csv")
        )
        ...
        try:
            ...
            if is_hits:
                ...
                for hit in items:
                    ...
                    q_idx = self._index_sequence(seq_query, "Q", accession=acc) if seq_query else None
    ...
    def preload_annotations(self):
        ...
        self.go_annotations = {}
        exclude_taxon_ids = {
            str(t) for t in getattr(self, "exclude_taxon_ids", []) if str(t).strip()
        }
        include_taxon_ids = {
            str(t) for t in getattr(self, "include_taxon_ids", []) if str(t).strip()
        }

        with self.engine.connect() as connection:
            for row in connection.execute(sql):
                row_taxonomy_id = None if row.taxonomy_id is None else str(row.taxonomy_id)
                if exclude_taxon_ids and row_taxonomy_id in exclude_taxon_ids:
                    continue
                if include_taxon_ids and row_taxonomy_id not in include_taxon_ids:
                    continue
    ...
    def _index_sequence(self, seq: str, which: str, accession: str | None = None) -> int | None:
        """
        ...
        accession : str | None
            Parsed query accession associated with the sequence. Used only for
            query ('Q') indexing so a mapping file can be exported later.
        ...
        if which == 'Q':
            if seq in self._q_seq_to_idx:
                idx = self._q_seq_to_idx[seq]
                if accession:
                    self._q_idx_to_accessions[idx].add(accession)
                return idx
            idx = len(self._q_idx_to_seq)
            self._q_seq_to_idx[seq] = idx
            self._q_idx_to_seq.append(seq)
            self._q_idx_to_accessions.append(set())
            if accession:
                self._q_idx_to_accessions[idx].add(accession)
            return idx
        elif which == 'R':
    ...
    def _flush_sequences_fasta(self) -> str:
        ...
        self._flush_query_index_mapping()
        return path

    def _flush_query_index_mapping(self) -> str:
        """
        Write a mapping between query FASTA indices (Q*) and parsed accessions.

        Notes
        -----
        Query indices are sequence-based. If multiple query accessions share the
        same sequence, they will map to the same Q index and will be exported as
        a semicolon-separated accession list.
        """
        os.makedirs(self.experiment_path, exist_ok=True)
        path = self.query_index_mapping_path
        with open(path, "w", newline="", encoding="utf-8") as handle:
            writer = csv.writer(handle)
            writer.writerow(["query_idx", "query_fasta_id", "accessions", "sequence_length"])
            for i, seq in enumerate(self._q_idx_to_seq):
                accessions = sorted(self._q_idx_to_accessions[i]) if i < len(self._q_idx_to_accessions) else []
                writer.writerow([i, f"Q{i}", ";".join(accessions), len(seq)])
        self.logger.info(
            "Query index mapping written → %s | query_indices=%d",
            path, len(self._q_idx_to_seq)
        )
        return path
