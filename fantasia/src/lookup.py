...
class EmbeddingLookUp(QueueTaskInitializer):
    ...
    def lookup_table_into_memory(self):
        ...
        """
        try:
            self.logger.info("Loading sequence embeddings into memory...")
            ...
            for task_name, model_info in self.types.items():
                embedding_type_id = model_info["id"]
                self.logger.info(f"Fetching embeddings for model '{task_name}' (ID: {embedding_type_id})")
                ...
                if isinstance(entry_limit, int) and entry_limit > 0:
                    self.logger.info(f"Applying SQL limit of {entry_limit} for model '{task_name}'")
                    query = query.limit(entry_limit)

                results = query.all()
                if not results:
                    self.logger.warning(f"No embeddings found for model '{task_name}' (ID: {embedding_type_id})")
                    continue
                ...
                self.logger.info(f"Loaded {len(sequence_ids)} embeddings for model '{task_name}'.")

        except Exception as e:
            self.logger.error(f"Error while loading embeddings into memory: {e}")
            raise
